...
import { MOCK_SUBMODEL_TEMPLATES, MOCK_CATEGORIES } from "@/lib/mock-data";
...
export default function SubmodelPage() {
  ...
  const filtered = useMemo(() => {
    return MOCK_SUBMODEL_TEMPLATES.filter((m) => {
      const matchKey =
        !searchKey ||
        m.submodel_name.toLowerCase().includes(searchKey.toLowerCase()) ||
      ...
      <div className="border-b border-border bg-background px-6 py-4">
        <div className="mx-auto max-w-screen-2xl">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-bold text-foreground">Submodel Template</h1>
              <nav className="mt-0.5 flex items-center gap-1.5 text-xs text-muted-foreground">
                <Link href="/" className="hover:text-foreground">Home</Link>
                <span>/</span>
                <span>Submodel Template</span>
              </nav>
            </div>
        ...
        {layoutType === "grid" ? (
          ...
          <div className="rounded-lg border border-border overflow-hidden">
            <Table>
              ...
              <TableBody>
                {filtered.map((model) => (
                  <TableRow key={model.submodel_seq}>
                    <TableCell>
